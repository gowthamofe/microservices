pipeline {
    ...
    stages {
        stage('SCM Checkout'){
            steps {
            git branch: 'main', url: 'https://github.com/gowthamofe/microservices.git'
            sh 'ls'
            }
        }
                        ...
                        script{
                            dir('wishlist-microservice-python') {
                                ...
                                withSonarQubeEnv('sonar-pro') {
                                    ...
                                    -D sonar.host.url=http://13.126.149.206:9000/"""
                                }
                            }
                        }
                    }
                ...
                parallel (
                    'docker login': {
                        withCredentials([string(credentialsId: 'dockerPass', variable: 'dockerPassword')]) {
                            sh "docker login -u gowthamofe -p ${dockerPassword}"
                        }
                    },
                    'ui-web-app-reactjs': {
                        dir('ui-web-app-reactjs'){
                            sh """
                            docker build -t gowthamofe/ui:v1 .
                            docker push gowthamofe/ui:v1
                            docker rmi gowthamofe/ui:v1
                            """
                        }
                    },
                    'zuul-api-gateway' : {
                        dir('zuul-api-gateway'){
                            sh """
                            docker build -t gowthamofe/api:v1 .
                            docker push gowthamofe/api:v1
                            docker rmi gowthamofe/api:v1
                            """
                        }
                    },
                    'offers-microservice-spring-boot': {
                        dir('offers-microservice-spring-boot'){
                            sh """
                            docker build -t gowthamofe/spring:v1 .
                            docker push gowthamofe/spring:v1
                            docker rmi gowthamofe/spring:v1
                            """
                        }
                    },
                    'shoes-microservice-spring-boot': {
                        dir('shoes-microservice-spring-boot'){
                            sh """
                            docker build -t gowthamofe/spring:v2 .
                            docker push gowthamofe/spring:v2
                            docker rmi gowthamofe/spring:v2
                            """
                        }
                    },
                    'cart-microservice-nodejs': {
                        dir('cart-microservice-nodejs'){
                            sh """
                            docker build -t gowthamofe/ui:v2 .
                            docker push gowthamofe/ui:v2
                            docker rmi gowthamofe/ui:v2
                            """
                        }
                    },
                    'wishlist-microservice-python': {
                        dir('wishlist-microservice-python'){
                            sh """
                            docker build -t gowthamofe/python:v1 .
                            docker push gowthamofe/python:v1
                            docker rmi gowthamofe/python:v1
                            """
                        }
                    }
                )
